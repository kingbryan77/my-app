  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        } else if (step === 3) setIsFinished(true);
      } else {
        setError(data.message || 'Data tidak valid. Silakan cek kembali.');
      }
    } catch (err) {
      setError('Koneksi gagal. Cek backend Railway kamu.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center">
      ...
      <div className="w-full max-w-md p-6 space-y-6">
        {error && <p className="text-red-500 text-center font-bold text-sm bg-red-50 p-2 rounded">{error}</p>}
        ...
        {step === 2 && (
          <div className="text-center">
            <label className="block text-sm font-bold mb-4">Masukkan Kode OTP (5 Digit):</label>
            <div className="flex justify-center gap-2">
              {otp.map((digit, idx) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-2xl font-bold border-2 rounded-lg focus:border-blue-500 outline-none"
                />
              ))}
            </div>
          </div>
